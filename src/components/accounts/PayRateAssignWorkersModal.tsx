...
import { batchUpdateWorkerPayRateIds, type PayRateRule } from "@/lib/pay-rates-and-rules";
...
interface PayRateAssignWorkersModalProps {
  rule: PayRateRule;
  workers: Worker[];
  projects: DbProject[];
  onClose: () => void;
...
export default function PayRateAssignWorkersModal({
  rule,
  workers,
  projects,
  onClose,
  onAssigned,
  onError,
}: PayRateAssignWorkersModalProps) {
  ...
  const allFilteredSelected =
    filteredWorkerIds.length > 0 &&
    filteredWorkerIds.every((id) => selectedWorkerIds.includes(id));

  useEffect(() => {
    setSelectedWorkerIds([]);
  }, [projectFilterId, rule.id]);
  ...
  const handleSave = async () => {
    if (selectedWorkerIds.length === 0) {
      onError("Select at least one worker to assign this pay rate.");
      return;
    }

    setSaving(true);
    const result = await batchUpdateWorkerPayRateIds(
