...
interface SwmsDeleteConfirmModalProps {
  title: string;
  onClose: () => void;
...
export default function SwmsDeleteConfirmModal({
  title,
  onClose,
  onConfirm,
  deleting,
}: SwmsDeleteConfirmModalProps) {
  return (
    <div className={modalOverlayClass} onClick={onClose}>
      ...
      >
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <h2 className="text-lg font-bold text-slate-900">Delete SWMS</h2>
            <p className="mt-2 text-sm text-slate-600">
              Are you sure you want to permanently delete{" "}
              <span className="font-semibold text-slate-900">{title}</span>? This
              will also remove all associated signatures and assignments.
            </p>
          </div>
