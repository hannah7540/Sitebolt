...
export default function PlantArchiveModal({
  unitNumber,
  onClose,
  onConfirm,
  saving,
}: PlantArchiveModalProps) {
  const [reason, setReason] = useState<PlantArchiveReason>("Sold");
    ...
    <div className={modalOverlayClass} onClick={onClose}>
      ...
      >
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <h2 className="text-lg font-bold text-slate-900">Archive Asset</h2>
            <p className="mt-2 text-sm text-slate-600">
              Archive <span className="font-semibold text-slate-900">{unitNumber}</span>?
              Archived assets stay in the Archived tab and can be restored later.
            </p>
          </div>
        ...
        <label className="block">
          ...
          <select
            ...
            value={reason}
            onChange={(event) =>
              setReason(event.target.value as PlantArchiveReason)
            }
            disabled={saving}
          >
            {PLANT_ARCHIVE_REASONS.map((option) => (
