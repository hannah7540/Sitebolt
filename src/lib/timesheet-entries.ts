...
function normalizeActivities(raw: unknown): TimesheetActivitySlot[] {
  ...
  return raw
    .map((item) => {
      ...
      } satisfies TimesheetActivitySlot;
    })
...
export async function saveWorkerTimesheetEntry(
  ...
  if (input.activities.length === 0) {
    return { error: "Add at least one work activity.", data: null };
  }

  const totals = calculateDailyTotalsFromSlots(input.activities, input.breaks);
  ...
  const projectName = getProjectDisplayName(projectId) || "General / Unassigned";
  ...
  const fullPayload = buildPayload(input, projectId, projectName, totals, signatureUrl);
